...
def plot_rects(img, region_block_yxs_ori, region_block_yxs_mer, txt_file):
	...
	for i in range(len(region_block_yxs_mer)):
		...
		# mask1=cv2.rectangle(det_img, (x1, y1),  (x2, y2), (0, 255, 255), 3)		
		# txt_record = str(x1)+','+ str(y1)+','+str(x2)+','+str(y2)
		# if (y02 - y01) <= 0.5 * (y2 - y1):			
		# 	mask1 = cv2.rectangle(det_img, (x01, int(y01 * 0.98)), (x02, int(y02 * 1.02)), (0, 255, 255), 3)
		# 	txt_record = str(x01)+','+ str(int(y01 * 0.98))+','+str(x02)+','+str(int(y02 * 1.02))
		# else:
