...
class ImageLabelmapDataLayer(caffe.Layer):
  ...
  def forward(self, bottom, top):
    ...
    Load data
    """
    [imgfn, mskfn] = self.filelist[self.idx].split()
    [imgfn, mskfn] = join(self.root, imgfn), join(self.root, mskfn)
    assert isfile(imgfn) and isfile(mskfn), "File not exists!"
    img = cv2.imread(imgfn).astype(np.float32)
    msk = cv2.imread(mskfn, 0).astype(np.float32)
    if img.ndim == 2:
      img = img[:,:,np.newaxis]
