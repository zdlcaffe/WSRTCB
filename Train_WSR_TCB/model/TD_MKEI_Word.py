...
def fcn(split):
  ...
  if split=='train':
    ...
    n.data, n.label = L.Python(module='datalayer', layer='ImageLabelmapDataLayer', ntop=2, \
  ...
  if split=='train':
    n.loss1 = L.FocalLoss(n.upscore_dsn1, n.label, loss_param=dict(normalize=False))
  if split=='test':
  ...
  if split=='train':
    n.loss2 = L.FocalLoss(n.upscore_dsn2, n.label, loss_param=dict(normalize=False))
  if split=='test':
  ...
  if split=='train':
    n.loss3 = L.FocalLoss(n.upscore_dsn3, n.label, loss_param=dict(normalize=False))
  if split=='test':
  ...
  if split=='train':
    n.loss4 = L.FocalLoss(n.upscore_dsn4, n.label, loss_param=dict(normalize=False))
  if split=='test':
  ...
  if split=='train':
    n.loss5 = L.FocalLoss(n.upscore_dsn5, n.label, loss_param=dict(normalize=False))
  elif split=='test':
  ...
  if split=='train':
    n.loss_fuse = L.FocalLoss(n.upscore_fuse, n.label, loss_param=dict(normalize=False))
  return n.to_proto()

def make_net():
  with open('model/TD_train.pt', 'w') as f:
    f.write(str(fcn('train')))
  with open('model/TD_test.pt', 'w') as f:
    f.write(str(fcn('test')))
def make_solver():
  sp = {}
  sp['net'] = '"model/TD_train.pt"'
  sp['base_lr'] = '0.00000001'
  sp['lr_policy'] = '"step"'
  sp['momentum'] = '0.9'
  ...
  sp['display'] = '10'
  sp['snapshot'] = '5000'
  sp['snapshot_prefix'] = '"snapshot/TD_MKEI_Word"'
  sp['gamma'] = '0.1'
  sp['max_iter'] = '400000'
  sp['solver_mode'] = 'GPU'
  f = open('model/TD_solver.pt', 'w')
